...
def flash_array_credential_count(
    ...
    with HealthCheckRuntime(
        ...
        health_check_name=HealthCheckName.AIRSTORE_CREDENTIAL_COUNT,
        killswitch_getter=lambda: FeatureValueHealthChecksFeatures().get_healthchecksfeatures_disable_airstore_credential_count(),
    ) as rt:
        try:
            proc_out: PipedShellCommandOut = obj.get_credential_count(
                timeout, rt.logger
            )
